...
\textbf{Code release.} Experiment code, raw per-seed payoff matrices, and analysis scripts are released at \url{https://anonymous.4open.science/r/fl-vopd} (anonymized for review; de-anonymized at camera-ready). The release includes the $\tau$-sweep per-seed payoffs (\texttt{results/cifar10\_tau\_sweep/}), 10-seed per-mode CIFAR-10 data (\texttt{results/cifar10\_10seeds/}), and the per-round ASR logs for the persistence-model observations (\texttt{results/persistence/}).
...
\input{../results/cifar10_fltrust_pilot/lambda_c_sweep_table.tex}
