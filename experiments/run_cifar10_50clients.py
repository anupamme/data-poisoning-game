"""
N=50 client CIFAR-10 scale validation (4x5 game, 5 seeds).
Attacks: no_attack, backdoor_pixel, model_scaling, dba
Defenses: fedavg, norm_clip, rfa, trimmed_mean, coord_median

Mirrors run_cifar10_100clients_rich.py exactly except num_clients=50,
clients_per_round=10 (maintaining 20% adversarial fraction).

Output: results/cifar10_50clients/
"""
import sys, os, json
import numpy as np
...
fl_config = FLConfig(num_clients=50, clients_per_round=10, num_rounds=50)
...
output_dir = os.path.join(base_dir, "results", "cifar10_50clients")
os.makedirs(output_dir, exist_ok=True)

print(f"N=50 CIFAR-10 scale validation: 4x5 game, {len(SEEDS)} seeds")
...
for seed in SEEDS:
    ...
    # Record binding cell ASR for mechanism analysis
    asr_binding = None
    ...
    print(f"  Seed {seed}: VoPD={best_vopd:.4f}, mixed={is_mixed}, "
          f"ASR(scaling,fedavg)={asr_str}, "
          f"diagnostic_correct={all_correct}")

print(f"\n=== N=50 CIFAR-10 Summary ===")
